...
export function LinkRow({ title, detail, icon, onPress }: { title: string; detail?: string; icon: IconName; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={[ui.row, { minHeight: 62 }]}><Icon name={icon} /><View style={{ flex: 1 }}><Body strong>{title}</Body>{detail && <Body>{detail}</Body>}</View><Icon name="chevron-right" size={22} /></Pressable>;
}
