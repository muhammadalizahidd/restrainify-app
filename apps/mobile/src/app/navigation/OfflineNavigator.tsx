...
export function OfflineNavigator() {
  ...
  if (!snapshot) content = error ? <><Heading title="Your local data needs attention" /><Body>{error}</Body><Button title="Try again" onPress={() => void refresh()} /></> : <Loading />;
    ...
    case "recovery-progress": content = <RecoveryProgressScreen open={open} onBack={back} />; break;
    case "screen-time": content = <ScreenTimeScreen open={open} onBack={back} />; break;
    ...
    case "burst": content = <BurstActiveScreen open={open} onBack={back} />; break;
    case "burst-outcome": content = <BurstOutcomeScreen open={open} onBack={back} />; break;
  ...
  return <View style={{ flex: 1, backgroundColor: palette.backgroundPrimary }}><StatusBar barStyle={dark ? "light-content" : "dark-content" } backgroundColor={palette.backgroundPrimary} translucent /><View style={{ height: topInset, backgroundColor: palette.backgroundPrimary, width: "100%" }} /><KeyboardAvoidingView style={{ flex: 1 }} behavior="height"><ScrollView key={route} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 12, paddingBottom: 24, gap: 12 }} showsVerticalScrollIndicator={false}>
    {snapshot?.settings.onboardingComplete && !tabs.some(tab => tab.route === route) && route !== "permissions" && route !== "protection-health" && route !== "burst" && route !== "burst-outcome" && route !== "recovery-progress" && route !== "screen-time" && <Pressable accessibilityLabel="Go back" onPress={back} style={{ flexDirection: "row", gap: 6, alignItems: "center", minHeight: 44 }}><Icon name="arrow-left" /><Body>Back</Body></Pressable>}
